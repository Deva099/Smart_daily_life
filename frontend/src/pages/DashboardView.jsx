...
const DashboardView = ({ setActiveView }) => {
  ...
  const renderMobileView = () => (
    <div className="view-section" style={{ animation: 'fadeIn 0.6s ease-out' }}>
      ...
      <div style={{ display: 'flex', gap: '1.2rem', overflowX: 'auto', paddingBottom: '1.5rem', marginBottom: '1.5rem', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
        {habits.map(h => (
          <div key={h._id} className="card" style={{ minWidth: '180px', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border-color)', background: 'var(--surface-color)' }}>
  ...
  const renderDesktopView = () => (
    <div className="view-section" style={{ animation: 'fadeSlideUp 0.5s ease-out', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
        <div className="card shadow-hover" style={{ padding: '1.75rem', cursor: 'pointer', borderLeft: '4px solid var(--accent-primary)', background: 'var(--bg-card)' }} onClick={() => setActiveView?.('tasks')}>
          <div className="flex justify-between items-center mb-4">
        ...
        <div className="card shadow-hover" style={{ padding: '1.75rem', background: 'var(--accent-primary-light)', border: '1px solid var(--accent-primary)', position: 'relative', overflow: 'hidden' }}>
          <div className="flex justify-between items-center mb-4">
          ...
          <Zap size={60} color="var(--accent-primary)" style={{ position: 'absolute', bottom: '-10px', right: '-10px', opacity: 0.1 }} />
        </div>

        <div className="card shadow-hover" style={{ padding: '1.75rem', borderLeft: '4px solid var(--accent-secondary)', background: 'var(--bg-card)' }}>
          <div className="flex justify-between items-center mb-4">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '2.5rem' }}>
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1.25rem' }}>
            {habits.map(h => (
              <div key={h._id} className="card shadow-hover" style={{ padding: '1.5rem', transition: 'all 0.3s ease', background: 'var(--bg-card)' }}>
                <div className="flex justify-between items-center mb-4">
        ...
        <div>
           ...
           <div className="card text-center" style={{ padding: '3rem 2rem', background: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
  ...
  return (
    <>
      ...
      {isAddingHabit && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(11, 14, 20, 0.85)', zIndex: 4000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', backdropFilter: 'blur(10px)' }}>
          <div className="card shadow-lg flex flex-col gap-6" style={{ width: '100%', maxWidth: '420px', background: 'var(--bg-card)', padding: '2rem', border: '1px solid var(--border-color)' }}>
            ...
            <form onSubmit={handleAddHabit} className="flex flex-col gap-5">
              <div>
                ...
                <input type="text" autoFocus required placeholder="e.g. Morning Meditation" value={newHabitName} onChange={e => setNewHabitName(e.target.value)} style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--surface-color)', color: 'var(--text-primary)' }} />
              </div>
              <div className="flex gap-4">
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>Time</label>
                  <input type="time" required value={newHabitTime} onChange={e => setNewHabitTime(e.target.value)} style={{ width: '100%', padding: '0.6rem', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--surface-color)', color: 'var(--text-primary)' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>Repeat</label>
                  <select value={newHabitRepeat} onChange={e => setNewHabitRepeat(e.target.value)} style={{ width: '100%', padding: '0.6rem', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--surface-color)', color: 'var(--text-primary)' }}>
                    <option value="daily">Daily</option>
                    <option value="weekly">Weekly</option>
                  </select>
                </div>
              </div>
              <button type="submit" className="btn btn-primary" style={{ padding: '1.1rem', fontSize: '1.1rem', fontWeight: 700, borderRadius: '12px', background: 'var(--accent-primary)', border: 'none', color: 'white', cursor: 'pointer' }}>Start Habit</button>
            </form>
          </div>
        </div>
      )}
