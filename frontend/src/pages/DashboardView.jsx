  ...
  const renderMobileView = () => (
    <div className="view-section" style={{ animation: 'fadeIn 0.6s ease-out' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.2rem', marginBottom: '2.5rem' }}>
        <div className="card" onClick={() => setActiveView?.('habits')} style={{ padding: '1.8rem', background: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)', color: 'white', border: 'none', cursor: 'pointer', boxShadow: '0 12px 40px rgba(124, 58, 237, 0.4)' }}>
          ...
          <div style={{ width: '100%', background: 'rgba(255,255,255,0.2)', height: '8px', borderRadius: '10px', overflow: 'hidden' }}>
            <div style={{ width: `${habits.length > 0 ? (habits.filter(h => h.completed).length / habits.length) * 100 : 0}%`, height: '100%', background: 'white', borderRadius: '10px', transition: 'width 0.6s cubic-bezier(0.16, 1, 0.3, 1)' }} />
          </div>
        </div>
      ...
      <div className="card flex justify-between items-center" style={{ padding: '1.8rem', marginBottom: '2.5rem', border: '1px solid var(--border-color)' }}>
         ...
         <div style={{ position: 'relative', width: '90px', height: '90px', borderRadius: '50%', background: `conic-gradient(var(--accent-primary) ${progressPct}%, var(--border-solid) 0)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: '74px', height: '74px', borderRadius: '50%', background: 'var(--bg-color)', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 800 }}>{progressPct}%</div>
         </div>
      </div>

      <div className="flex justify-between items-center mb-4">
        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700 }}>My Habits</h3>
        <button onClick={() => setIsAddingHabit(true)} style={{ background: 'transparent', border: 'none', fontSize: '0.9rem', color: 'var(--accent-primary)', fontWeight: 700 }}>+ Add New</button>
      </div>
      <div style={{ display: 'flex', gap: '1.2rem', overflowX: 'auto', paddingBottom: '1.5rem', marginBottom: '1.5rem', scrollbarWidth: 'none' }}>
        {habits.map(h => (
          <div key={h._id} className="card" style={{ minWidth: '170px', padding: '1.5rem' }}>
             <div className="flex justify-between items-start mb-6">
               <div className="btn-icon-only" style={{ background: 'var(--bg-color)', width: '44px', height: '44px' }}>{getDynamicIcon(h.title)}</div>
               <div onClick={() => toggleHabit(h._id, h.completed)} style={{ width: '40px', height: '22px', borderRadius: '20px', background: h.completed ? 'var(--accent-primary)' : 'var(--border-solid)', position: 'relative', cursor: 'pointer' }}>
                 <div style={{ position: 'absolute', top: '3px', left: h.completed ? '21px' : '3px', width: '16px', height: '16px', borderRadius: '50%', background: 'white', transition: '0.3s' }} />
               </div>
             </div>
             <h4 style={{ margin: '0', fontSize: '1.1rem', fontWeight: 700 }}>{h.title}</h4>
             <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{h.streak} Streak 🔥</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setActiveView?.('tasks')} style={{ background: 'transparent', border: 'none', fontSize: '0.85rem', color: 'var(--text-muted)' }}>View All</button>
      </div>
        ...
        {tasks.map(t => (
          <div key={t._id} className="card flex items-center justify-between" style={{ padding: '1.1rem 1.5rem', background: 'var(--surface-color)' }}>
            <div className="flex items-center gap-4">
              <button onClick={() => toggleTask(t._id, t.completed)} style={{ width: '24px', height: '24px', borderRadius: '8px', border: t.completed ? 'none' : '2px solid var(--border-solid)', background: t.completed ? 'var(--accent-primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                 {t.completed && <Check size={16} color="white" />}
              </button>
              <span style={{ fontSize: '1rem', fontWeight: 600, textDecoration: t.completed ? 'line-through' : 'none', color: t.completed ? 'var(--text-muted)' : 'var(--text-primary)' }}>{t.title}</span>
            </div>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: t.priority === 'High' ? 'var(--danger)' : 'var(--success)' }} />
          </div>
        ))}
      </div>
      
      <button className="fab" onClick={() => setIsAddingHabit(true)} style={{ background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))', bottom: '100px' }}>
        <Plus size={32} />
      </button>
    </div>
  );
